...
class DenseLM2Model(DenseModel):
    def __init__(
            self,
            lm: PreTrainedModel,
            mlm_head: nn.Module,
    ...
    def forward(
        ...
        # mlm loss & encoding
        query_mask = copy.deepcopy(query)
        for k in query_mask:
            try:
                if 'input_ids_with_mask' in query_mask[k]:
                    query_mask[k]['input_ids'] = query_mask[k]['input_ids_with_mask']
            except:
                pass
        keys_mask = copy.deepcopy(keys)
        for k in keys_mask:
            try:
                if 'input_ids_with_mask' in keys_mask[k]:
                    keys_mask[k]['input_ids'] = keys_mask[k]['input_ids_with_mask']
            except:
                pass

        q_hidden2, q_reps2 = self.encode(query_mask)
        p_hidden2, p_reps2 = self.encode(keys_mask)

        if self.train_args.mlm_loss:
            q_prediction_scores = self.mlm_head(q_hidden2)
            k_prediction_scores = self.mlm_head(p_hidden2)
            q_mlm_loss = self.cross_entropy(q_prediction_scores.view(-1, self.lm_config.vocab_size), query['center_input']['labels'].view(-1))
            k_mlm_loss = self.cross_entropy(k_prediction_scores.view(-1, self.lm_config.vocab_size), keys['center_input']['labels'].view(-1))
            loss = loss + self.train_args.mlm_weight * (q_mlm_loss + k_mlm_loss)
...
class DenseLM2NModel(DenseModel):
    def __init__(
            self,
            lm: PreTrainedModel,
            mlm_head: nn.Module,
    ...
    def forward(
        ...
        n_num = 3
        bz, seq_len = query_mask['center_input']['input_ids'].shape

        query_mask['center_input']['input_ids'] = torch.cat((query_mask['center_input']['input_ids'], query_mask['neighbor_input']['input_ids'].view(bz,-1)[:,:n_num*seq_len]), 1)
        query_mask['center_input']['attention_mask'] = torch.cat((query_mask['center_input']['attention_mask'], query_mask['neighbor_input']['attention_mask'].view(bz,-1)[:,:n_num*seq_len]), 1)
        query_mask['center_input']['labels'] = torch.cat((query_mask['center_input']['labels'], query_mask['neighbor_input']['labels'].view(bz,-1)[:,:n_num*seq_len]), 1)

        keys_mask['center_input']['input_ids'] = torch.cat((keys_mask['center_input']['input_ids'], keys_mask['neighbor_input']['input_ids'].view(bz,-1)[:,:n_num*seq_len]), 1)
        keys_mask['center_input']['attention_mask'] = torch.cat((keys_mask['center_input']['attention_mask'], keys_mask['neighbor_input']['attention_mask'].view(bz,-1)[:,:n_num*seq_len]), 1)
        keys_mask['center_input']['labels'] = torch.cat((keys_mask['center_input']['labels'], keys_mask['neighbor_input']['labels'].view(bz,-1)[:,:n_num*seq_len]), 1)
        #######
        ...
        if self.train_args.mlm_loss:
            q_prediction_scores = self.mlm_head(q_hidden2)
            k_prediction_scores = self.mlm_head(p_hidden2)
            q_mlm_loss = self.cross_entropy(q_prediction_scores.view(-1, self.lm_config.vocab_size), query_mask['center_input']['labels'].view(-1))
            k_mlm_loss = self.cross_entropy(k_prediction_scores.view(-1, self.lm_config.vocab_size), query_mask['center_input']['labels'].view(-1))
            loss = loss + self.train_args.mlm_weight * (q_mlm_loss + k_mlm_loss)
...
class DenseMLMModel(DenseModel):
    def __init__(
            self,
            lm: PreTrainedModel,
            mlm_head: nn.Module,
